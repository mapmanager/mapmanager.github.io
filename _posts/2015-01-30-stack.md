...
Use keyboard <kbd>1</kbd>, <kbd>2</kbd>, and <kbd>3</kbd> to switch between channels one, two, and three. Color channels can also be switched using the <a href="#buttons-panel">buttons panel</a> or the mouse right-click menu.
...
Open the contrast bar with keyboard <kbd>c</kbd>, the right-click menu 'View - Contrast Bar', or use the <a href="#buttons-panel">buttons panel</a> 'control bar'.
...
The buttons panel provides a quick interface to perform actions on stack and [map plot](map-plot) windows. The actions provided in the buttons panel are also available from individual map manager windows using a mouse right-click.

Open the buttons panel from the main menu 'MapManager - Buttons' or use keyboard <kbd>ctrl</kbd>+<kbd>1</kbd>.

Once open, the buttons panel will automatically update based on the front-most Map Manager window. The 'stack' tab is for setting stack windows, the 'Obj Map' tab for setting [map plot](map-plot) windows and the 'Report' tab is for generating [reports][reports].
...
Open the histogram window with keyboard <kbd>h</kbd>, the right-click menu, or the <a href="#buttons-panel">buttons panel</a>.
...
Switch channels or to a sliding z-projection using the following table. This is also available in the <a href="#buttons-panel">buttons panel</a>.
...
Right-click on the image and you will get a contextual menu to activate the features described on this page. All the options available in the right-click menu can also be set using the <A HREF="#buttons-panel">buttons</A> panel.
...
Map Manager makes heavy use of keyboard shortcuts. All of these keyboard shortcuts are available in the mouse right-click menu (which also lists the corresponding keyboard shortcut) and in the <A HREF="#buttons-panel">buttons panel</a>.
...
Depending on the number of color channels in an image, color channels can be switch using keyboard <kbd>1</kbd>, <kbd>2</kbd>, and <kbd>3</kbd>. This includes single color channels, sliding z-projections, and merged RGB.
