...
Stack reports display the raw data for all lines and/or all annotations in a stack.

Stack reports can be generated directly from each [stack](stack) window.

#### Line Segments

- Keyboard 'e' In the 'Segments' section of the main stack db panel to display a summary of each segment.
- Keyboard 'E' In the 'Segments' section of the main stack db panel to display the entire line (all 3D points)

<p class="tip"><B>Tip.</B> Use keyboard '[' in any stack window to open and close the left stack db panel.</p>

#### Points (Annotations)

- Keyboard 'e' In the 'Points' section of the main stack db panel to display all annotations as a table.
...
To generate a segment report for each segment in a stack
 ...
 - In a [stack window][1], use the 'Segment report' button in the 'Reports' tab.
...
- Right-click a map in the time-series panel and select 'Survival Report'.
